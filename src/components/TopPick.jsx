...
import { ACCENT_HEX, ACCENT_MUTED_HEX } from '../lib/theme'
...
export default function TopPick({ signal }) {
  ...
  if (!signal) {
    return (
      <SpotlightCard
        icon={Sparkles}
        title="Top Pick"
        subtitle="No qualifying setups for this session."
        color={ACCENT_MUTED_HEX}
        className="top-pick top-pick--empty-shell fade-up"
      >
  ...
  return (
    <SpotlightCard
      icon={Sparkles}
      title="Top Pick"
      subtitle={`${signal.ticker.replace('.NS', '')} · ${signal.sector}`}
      color={ACCENT_HEX}
      className="top-pick fade-up"
    >
